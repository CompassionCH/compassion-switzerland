...
class FieldOffice(models.Model):
    ...
    country_info_pdf = fields.Binary(compute="_compute_country_info_pdf")
    country_info_filename = fields.Char(compute="_compute_info_country_filename")

    # French document
    fr_country_info_pdf = fields.Binary(
        string="Country information French", attachment=True
    )

    # German document
    de_country_info_pdf = fields.Binary(
        string="Country information German", attachment=True
    )

    # Italian document
    it_country_info_pdf = fields.Binary(
        string="Country information Italian", attachment=True
    )

    # English document
    en_country_info_pdf = fields.Binary(
        string="Country information English", attachment=True
    )
