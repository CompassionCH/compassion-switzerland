...
class SmsNotification(models.Model):
    ...
    state = fields.Selection(
        [("new", "Received"), ("success", "Success"), ("failed", "Failed")],
        default="new",
    )
    ...
    def create(self, vals):
        ...
        hook = self.env["sms.hook"].search([("name", "=ilike", vals["service"])])
        ...
        langs = (
            self.env["res.lang"].search(
                [("code", "=ilike", vals["language"] + "%")], limit=1
            )
            if "language" in vals and vals["language"]
            else ()
        )
    ...
    def run_service(self):
        ...
        if not self.hook_id:
            ...
            try:
                sms_answer = service()
                self.write({"state": "success", "answer": sms_answer})
            except Exception:
