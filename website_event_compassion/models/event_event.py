...
class Event(models.Model):
    ...
    valid_ticket_ids = fields.Many2many(
        "event.event.ticket", compute="_compute_valid_tickets", readonly=False
    )
    ...
    country_id = fields.Many2one(
        "res.country",
        "Country",
        related="compassion_event_id.country_id",
        readonly=False,
    )
    ...
    event_ticket_ids = fields.One2many(
        ...
        string="Event Ticket",
        copy=True,
        default=lambda self: self._default_tickets(),
        readonly=False,
    )

    def _compute_total_price(self):
        flight = self.env.ref("website_event_compassion.product_template_flight")
        ...
        for event in self:
            ...
            event.flight_price = sum(
                tickets.filtered(lambda t: t.product_id == flight_product).mapped(
                    "price"
                )
                or [0]
            )
    ...
    def _compute_valid_tickets(self):
        for event in self:
            event.valid_ticket_ids = event.mapped("event_ticket_ids").filtered(
                lambda t: not t.is_expired
                and (t.seats_available or t.seats_availability == "unlimited")
            )
    ...
    def _compute_registration_closed(self):
        for event in self:
            ...
            event.registration_closed = (
                ...
                or not event.valid_ticket_ids
            )

    def _compute_registration_not_started(self):
        for event in self:
            event.registration_not_started = (
                event.state == "draft" or not event.event_ticket_ids
            )
    ...
    def _compute_registration_full(self):
        for event in self:
            ...
            event.registration_full = (
                event.state == "confirm"
                and datetime.now() < start_date
                and not event.valid_ticket_ids
            )
    ...
    def _default_tickets(self):
        ...
        res = [{"name": _("Registration"), "product_id": product.id, "price": 0, }]
        room = self.env.ref("website_event_compassion.product_template_single_room")
        flight = self.env.ref("website_event_compassion.product_template_flight")
        trip = self.env.ref("website_event_compassion.product_template_trip_price")
        products = self.env["product.product"].search(
            [("product_tmpl_id", "in", (room + flight + trip).ids)]
        )
        res.extend(
            [
                {"name": product.name, "product_id": product.id, "price": 0, }
