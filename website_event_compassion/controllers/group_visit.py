...
class GroupVisitController(EventsController):
    ...
    def group_visit_step2(self, reg_uid, form_id=None, **kwargs):
        ...
        contract_form = self.get_form("event.registration", registration.id, **values)
        ...
        trip_form = self.get_form("event.registration", registration.id, **values)
        ...
        criminal_form = self.get_form("event.registration", registration.id, **values)
        ...
        if registration.stage_id == request.env.ref(
            ...
            return request.render("website_event_compassion.group_visit_step2", values)
        else:
            values = {
    ...
    def get_payment_form(self, reg_uid, form_model, **kwargs):
        ...
        if payment_form.form_success:
            ...
            return werkzeug.utils.redirect(payment_form.form_next_url(), code=303)
    ...
    def _get_group_visit_page_values(self, reg_uid, form_model=None, **kwargs):
        ...
        values.pop("edit_translations", False)
        values.update(
            {"registration": registration, "event": registration.compassion_event_id}
        )
        if form_model is not None:
            form_id = registration.id if form_model == "event.registration" else None
