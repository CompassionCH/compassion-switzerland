...
class TravelContractForm(models.AbstractModel):
    ...
    def form_msg_success_updated(self):
        return _("Thank you for having read and signed the travel " "agreements.")
    ...
    def form_before_create_or_update(self, values, extra_values):
        if extra_values.get("contract_agreement"):
            ...
            sign_task = self.env.ref("website_event_compassion.task_sign_travel")
...
class ChildProtectionForm(models.AbstractModel):
    ...
    final_question = fields.Selection(
        [
            (
                "yes_free",
                "Yes, in an open place and taking into account the "
                "indications of Compassion.",
            ),
            (
                "no",
                "No, the Compassion Charter does not allow me to be alone "
                "with a child in any situation. I must always be "
                "accompanied by someone.",
            ),
            (
                "yes_my",
                "Yes, if he or she is my sponsored child, I can speak "
    ...
    def form_after_create_or_update(self, values, extra_values):
        ...
        self.main_object.sudo().partner_id.agree_to_child_protection_charter()
...
class TripForm(models.AbstractModel):
    ...
    def form_before_create_or_update(self, values, extra_values):
        # Mark tasks as done
        completed_tasks = []
        if values.get("passport_number") and values.get("passport_expiration_date"):
            completed_tasks.append(
                (4, self.env.ref("website_event_compassion.task_passport").id)
            )
