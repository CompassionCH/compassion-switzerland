...
mock_get_staff = (
    "odoo.addons.child_compassion.wizards"
...
class TestWordpressConnector(BaseSponsorshipTest):
    ...
    def create_sponsorship_default(
        ...
        return self.env["recurring.contract"].create_sponsorship(
            child_local_id, form_data, lang, utm_source, utm_medium, utm_campaign
        )
    ...
    @mock.patch(mock_get_infos)
    @mock.patch(mock_project_infos)
    def test_existing_partner(self, project_infos, get_infos, update_hold, get_staff):
        """
        ...
        self.assertTrue(sponsorship_res)
        self.check_sponsorship(sponsorship_res, child, partner_id=self.michel.id)
