...
def copy_attachments(env, model_name):
    for record in env[model_name].search([]):
        attachment = env["ir.attachment"].search([("res_model", "=", model_name), ("res_id", "=", record.id), ("res_field", "=", "image")])
        if attachment:
            for image_size in ["image_1920", "image_1024", "image_512", "image_256", "image_128"]:
