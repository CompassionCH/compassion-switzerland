...
class PaymentAcquirerOgone(models.Model):
    ...
    def _get_ogone_urls(self, environment):
        ...
        return {
            "ogone_standard_order_url": f"https://e-payment.postfinance.ch/ncol/"
                                        f"{environment}/orderstandard_utf8.asp",
            "ogone_direct_order_url": f"https://e-payment.postfinance.ch/ncol/"
                                      f"{environment}/orderdirect_utf8.asp",
            "ogone_direct_query_url": f"https://e-payment.postfinance.ch/ncol/"
                                      f"{environment}/querydirect_utf8.asp",
            "ogone_afu_agree_url": f"https://e-payment.postfinance.ch/ncol/"
                                   f"{environment}/AFU_agree.asp",
        }
    ...
    def ogone_form_generate_values(self, values):
        ...
        temp_ogone_tx_values = {
            ...
            "OWNERCTY": values.get("partner_country")
            and values.get("partner_country").code
            or "",
            "OWNERTELNO": values.get("partner_phone"),
            "ACCEPTURL": "%s"
                         % urllib.parse.urljoin(base_url, OgoneCompassion._accept_url),
            "DECLINEURL": "%s"
                          % urllib.parse.urljoin(base_url,
                                                 OgoneCompassion._decline_url),
            "EXCEPTIONURL": "%s"
                            % urllib.parse.urljoin(base_url,
                                                   OgoneCompassion._exception_url),
            "CANCELURL": "%s"
                         % urllib.parse.urljoin(base_url, OgoneCompassion._cancel_url),
            "PARAMPLUS": "return_url=%s" % ogone_tx_values.pop("return_url")
...
class PaymentTransaction(models.Model):
    ...
    def _compute_payment_mode(self):
        for tx in self.filtered("postfinance_brand"):
            tx.payment_mode_id = tx.payment_mode_id.search(
                [("name", "ilike", tx.postfinance_brand)], limit=1
            )
    ...
    def push_s2s_to_wordpress(self, post_data):
        ...
        res = requests.get("https://" + host + "/confirmation-don", post_data)
