...
class OgoneCompassion(OgoneController):
    ...
    def ogone_form_feedback(self, **post):
        ...
        _logger.info(
            "Ogone: entering form_feedback with post data %s", pprint.pformat(post)
        )  # debug
        ...
        redirect = (
            post.pop("return_url", "/") + "?" + urllib.urlencode(encoded_dict(post))
        )
    ...
    def feedback(self, **kwargs):
        ...
        if res.response[0] == "ko":
            ...
            request.env["payment.transaction"].sudo().with_delay(
                eta=30
