...
class SurveyQuestion(models.Model):
    ...
    max_checked_option = fields.Integer(
        "Maximum answers", help="if set, maximum number of options allowed for the user"
    )
    ...
    def validate_multiple_choice(self, post, answer_tag):
        ...
        if self.max_checked_option:
            ...
            answer_candidates = {
                k: v for k, v in post.items() if k.startswith(answer_tag)
            }
...
class SurveyLabel(models.Model):
    ...
    value = fields.Html(
        "Suggested value", translate=True, required=True, sanitize=False
    )
        ...
        for user_input_line in self.user_input_line_ids:
            ...
            # 0 for unselected option and "quizz_mark" for selected option.
                ...
                if q.id not in all_options:
                    ...
                    all_options[q.id] = [
                        0
                        for _ in self.env["survey.label"].search(
                            [("question_id", "=", q.id)]
                        )
                    ]
                ...
                if q.comment_count_as_answer and user_input_line.answer_type == "text":
                    ...
                    all_options[q.id][
                        user_input_line.value_suggested.order_in_question
                    ] = user_input_line.quizz_mark
