...
class ResPartnerSegmentAffinity(models.Model):
    ...
    answer_matrix_engine = [
        [0.997150993572249, -0.232528927794568, 0.0413534389887897, 0.211672855669691, 0],
        [0.928637961891426, 1.07809459110299, 0.809374673519616, -0.353365425615717, 0],
        [-0.774287516323442, -0.353077503405008, -0.497598067137283, 0.0290171579471129, 0],
        [-0.828702041340668, -0.306263089055513, -0.25127881181768, 0.137330144026763, 0],
        [-0.384199925830168, 0.443946593143302, -0.302689514147442, 0.253657198644858, 0],
        ...
        [-0.0326103888365477, -2.99571602280864, -2.69104850676188, -1.75084871433893, 0],
        [-0.783439038458744, 0.917536337079493, 0.86575431420832, 2.93637326365812, 0],
        [-0.566278267023883, -0.893538321476831, -3.80511027210482, -4.14668710742975, 0],
    ...
    answer_constant_engine = [
        -6.7614377947751, -8.85904330388748, -5.02071420157718, -6.23710433298537, 0
    ]
    ...
    def segment_affinity_engine(self, answer_as_array, partner_id):
        ...
        scaled_engine = [list(map(lambda x: answer_as_array[i] * x, self.answer_matrix_engine[i])) for i in
                         range(len(self.answer_matrix_engine))]
        ...
        segment_score = [sum(x) for x in zip([sum(x) for x in zip(*scaled_engine)], self.answer_constant_engine)]
