...
class AdvocateDetails(models.Model):
    ...
    def advocate_cron(self):
        ...
        dates = [three_open_days]

        # on tuesday you'll get not only the birthdays of friday but also
        # the birthdays of the upcoming weekend
        if three_open_days.weekday() == 4:
            dates.append(three_open_days + DateOffset(n=1))
            dates.append(three_open_days + DateOffset(n=2))

        birthday_advocates = self.search(
            [
                ("state", "in", ["active", "on_break"]),
                ("birthdate", "in", [d.strftime("-%m-%d") for d in dates]),
            ]
        )
