...
class PartnerCommunication(models.Model):
    ...
    #                                 FIELDS                                 #
    ##########################################################################
    product_id = fields.Many2one("product.product", "A4 + payment slip", readonly=False)
        ...
        if bvr_both:
            ...
            for bvr in bvr_both:
                if origin == "both_email" or (
                        origin == "both_print"
                        and bvr.report_id != self.env.ref(
                            "report_compassion.report_a4_bvr")
                ):
                    # email part
                    self._put_bvr_in_attachments(bvr, background=origin == "both_email")
