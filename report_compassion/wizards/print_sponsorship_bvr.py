...
class PrintSponsorshipBvr(models.TransientModel):
    ...
    def print_report(self):
        ...
        if self.pdf:
            name = records.name if len(records) == 1 else _("sponsorship payment slips")
            self.pdf_name = name + ".pdf"
            self.pdf_download = base64.b64encode(
                self.env["report"]
                .with_context(must_skip_send_to_printer=True)
                .get_pdf(records.ids, self.paper_format, data=data)
            )
            self.state = "pdf"
            return {
        ...
        return self.env["report"].get_action(records.ids, self.paper_format, data)
    ...
    def print_report(self):
        ...
        if self.pdf:
            self.pdf_download = base64.b64encode(
                self.env["report"]
                .with_context(must_skip_send_to_printer=True)
                .get_pdf(records.ids, report, data=data)
            )
            self.state = "pdf"
            return {
                "name": "Download report",
        ...
        return self.env["report"].get_action(records.ids, report, data)
