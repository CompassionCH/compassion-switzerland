...
class PaymentSlipController(Controller):
    ...
    def get_payment_slip(
        self, partner_uuid, fund_id, sponsorship_id=None, fund_amount=0
    ):
        """
        ...
        product = (
            ...
            .sudo()
            .search([("fund_id", "=", fund_id)], limit=1)
        )
        ...
        if not sponsorship_id:
            ...
            if product.categ_id != fund:
                raise BadRequest()
            wizard = (
                request.env["print.bvr.fund"]
                .sudo()
                .create({"product_id": product.id, "pdf": True, "amount": fund_amount})
