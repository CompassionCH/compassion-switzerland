...
BEGIN
    ...
    SET signature = (SELECT '<span><b>' || p.preferred_name || ' ' || p.lastname || '</b></span><br>' || '<span>Compassion Suisse</span><br>' || '<span>Rue Galilée 3</span><br>' || '<span>CH-1400 Yverdon-les-Bains</span><br>'
    || '<span>tel: +41 24 434 21 24</span><br>' || '<span><a style="color:#0054a6" href="https://www.compassion.ch/" target="_blank">www.compassion.ch</a></span><br>'
    || '<span><a href="https://compassion.ch/mailbanner_fr" target="_blank"><img width="100%" style="max-width: 640px" src="' || email_banner_url || 'fr_CH.png"></img></a></span>'
    FROM res_users u INNER JOIN res_partner p ON u.partner_id = p.id where u.id = users.id)
    ;
    ...
    SELECT 	nextval('ir_translation_id_seq'::regclass) as id,
        ...
        (SELECT '<span><b>' || p.preferred_name || ' ' || p.lastname || '</b></span><br>' || '<span>Compassion Suisse</span><br>' || '<span>Rue Galilée 3</span><br>' || '<span>CH-1400 Yverdon-les-Bains</span><br>' || '<span>tel: +41 24 434 21 24</span><br>' || '<span><a style="color:#0054a6" href="https://www.compassion.ch/" target="_blank">www.compassion.ch</a></span><br>'
         || '<span><a href="https://compassion.ch/mailbanner_fr" target="_blank"><img width="100%" style="max-width: 640px" src="' || email_banner_url || 'fr_CH.png"></img></a></span>' FROM res_users u INNER JOIN res_partner p ON u.partner_id = p.id where u.id = users.id) as src,
        ...
        '<span><a href="' || email_banner_url_open || '" target="_blank"><img width="100%" style="max-width: 640px" src="' || email_banner_url || wLang || '.png"></img></a></span>' as value,
        'model' as model
