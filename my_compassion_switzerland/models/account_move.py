    ...
    def action_post(self):
        ...
        for invoice in self:
            ...
            if (invoice.move_type != 'out_invoice' or
                    invoice.payment_state != 'not_paid' or
                    not getattr(invoice, 'recurring_invoicer_id', False)):
                continue
            ...
            # --- CRITICAL FIX: Ignore invoices from old groups ---
            ...
            if group.payment_token_id and group.payment_token_id.active:
                ...
                if invoice.transaction_ids.filtered(lambda t: t.state in ['done', 'authorized', 'pending']):
                    _logger.info(f"Skipping auto-charge for {invoice.name}: Valid transaction already exists.")
                    continue
