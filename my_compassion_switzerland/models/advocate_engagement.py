...
class AdvocateEngagement(models.Model):
    ...
    my_compassion_alt_text = fields.Char(
        translate=True,
        help="Alt text for the image of the engagement type visible on the MyCompassion website",
    )

    my_compassion_external_link = fields.Char(
        help="External link for the engagement type which redirects from MyCompassion website to another website",
    )

    my_compassion_internal_link = fields.Char(
        help="Internal link for the engagement type which sends a request to the Odoo server",
    )