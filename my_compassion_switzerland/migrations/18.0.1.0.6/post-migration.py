...
def migrate(cr, version):
    ...
    - Separately, a real payment fails at account.payment creation with
      ...
      linking for real. Confirmed this is the correct fix (not a mix-up
      with the online payment method / payment mode concepts) after
      checking with the accounting side: the "Web" journal (no attached
      bank account) is the intended online-provider clearing journal,
      distinct from the real "Postfinance CHF" bank journal used for
      actual bank statement reconciliation.
    """
