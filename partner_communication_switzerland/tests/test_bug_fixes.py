...
class TestSponsorship(BaseSponsorshipTest):
    ...
    @mock.patch(mock_update_hold)
    def test_wrpr_letter_reminder_skips_recent_sponsorship_t3306(self, hold_mock):
        """A W&P sponsorship started less than 545 days ago is not yet due a
        letter-writing reminder, and must be excluded by the (cheaper) SQL
        start_date filter instead of a per-record computed-field check (T3306).
        """
        hold_mock.return_value = True

        child = self.create_child(self.ref(13))
        sponsorship = self.create_contract(
            {
                "partner_id": self.michel.id,
                "group_id": self.sp_group.id,
        ...
        sponsorship.write(
            {
                "state": "active",
                "start_date": fields.Datetime.now() - relativedelta(days=100),
            }
        )

        self.env["recurring.contract"].send_wrpr_letter_reminder()
        self.assertEqual(
            self.env["queue.job"].search_count(
                [
                    (
                        "identity_key",
                        "=",
                        "recurring.contract.send_wrpr_letter_reminder."
                        f"{sponsorship.id}",
                    )
                ]
            ),
            0,
            "a recently started sponsorship must not be reminded",
        )
