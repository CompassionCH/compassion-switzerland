  ...
  steps: () => [
    ...
    },
    {
      ...
      async run() {
        ...
        const [donationLineIds, pending] = await Promise.all([
          callKw("account.move.line", "search", [
            [
              ["partner_id.lastname", "=", DONOR_LASTNAME],
              ["product_id.requires_thankyou", "=", true],
            ],
          ]),
          callKw("queue.job.replacement", "search_read", [
            [
              ["state", "=", "pending"],
              ["job_function", "=", "generate_thank_you"],
            ],
            ["res_ids"],
          ]),
        ]);

        const donationJobIds = pending
          .filter((job) =>
            JSON.parse(job.res_ids).some((id) => donationLineIds.includes(id)),
          )
          .map((job) => job.id);
