...
class ChangeTextWizard(models.TransientModel):
    ...
    def _default_state(self):
        ...
        if event:
            if len(event) > 1:
                raise odooWarning(_("You can only change one event at a time."))
            return "event"
        else:
            return "edit"
    ...
    def update(self):
        ...
        if event:
            ...
            if (
                partner
                and self.ambassador_text != partner.advocate_details_id.thank_you_quote
            ):
                partner.advocate_details_id.thank_you_quote = self.ambassador_text
            ...
            for comm in communications:
                comm.body_html = new_texts[comm.id].replace(event.name, self.event_name)
                if partner:
    ...
    def get_preview(self):
        if self.state == "event":
            ...
            self.write({"state": "preview", "preview": preview})
            return self._reload()
        else:
