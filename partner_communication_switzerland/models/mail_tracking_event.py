...
class MailTrackingEvent(models.Model):
    ...
    def process_unsub(self, tracking_email, metadata):
        ...
        tracking_email.partner_id.message_post(
            body=_("Partner Unsubscribed from marketing e-mails"), subject=_("Opt-out")
        )
    ...
    def _invalid_email(self, tracking_email):
        ...
        correspondence_email = tracking_email.filtered(
            lambda x: x.mail_id.model == "partner.communication.job"
            and self.env["partner.communication.job"].browse(x.mail_id.res_id).model
            == "correspondence"
        )
        ...
        onboarding_welcome_email = tracking_email.filtered(
            lambda x: x.mail_id.model == "partner.communication.job"
            and self.env["partner.communication.job"].browse(x.mail_id.res_id).config_id
            == onboarding_welcome_config
        )
