...
class StatementCompletionRule(models.Model):
    ...
    def get_from_partner_ref(self, stmts_vals, st_line):
        ...
        partner_ref = ref[
            ref_index_end - 7 : ref_index_end
        ]  # get standard 7 numbers ref
        old_partner_ref = ref[
            ref_index_end - 6 : ref_index_end
        ]  # get legacy 6 numbers ref
        partner_obj = self.env["res.partner"]
        partner = partner_obj.search(
            ["|", ("ref", "=", partner_ref), ("ref", "=", old_partner_ref)]
        )
        if not partner:
