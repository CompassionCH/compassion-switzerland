...
class SendPostfinanceLinkWizard(models.TransientModel):
    ...
    invoice_ids = fields.Many2many(
        'account.invoice', 'account_invoice_send_postfinance_link_wizard_rel',
    ...
    origin = fields.Char(
        compute='_compute_origin', inverse='_inverse_origin')
    ...
    def _compute_origin(self):
        for wizard in self:
            wizard.origin = ' '.join(wizard.invoice_ids.filtered('origin')
                                     .mapped('origin'))
    ...
    def generate_communication(self):
        ...
        config_id = self.env.ref(
            'invoice_postfinance_payment_controller'
            '.invoice_online_payment_config'
        ).id
