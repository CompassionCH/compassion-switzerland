...
class MonitorCorrectErrors(models.Model):
    ...
    def _fix_hold(self):
        ...
        list_of_child_without_hold = (
            self.env["recurring.contract"]
                .search(
                [
                    ("state", "in", ["waiting", "mandate"]),
                    ("type", "=", "S"),
                    ("child_id.hold_id", "=", False),
                ]
            )
            .mapped("child_id")
        )
    ...
    def _fix_sbc(self):
        ...
        list_correspondence_translation_queue_error = self.env["correspondence"].search(
            [
        ...
        list_correspondence_scan_error = self.env["correspondence"].search(
            [("state", "=", "Received in the system"), ("status_date", "<", limit_date)]
        )
    ...
    def _create_reservation(self, child, error_log):
        ...
        previous_reservation = self.env["compassion.reservation"].search(
            [("child_global_id", "=", child.global_id), ("state", "!=", "expired")]
        )
    ...
    def _send_mail(self):
        ...
        for error_type in self.env["error.log"].get_error_type():
            ...
            for error in self.error_ids.filtered(lambda e: e.error_type == error_type):
                message_body += "<tr>"
